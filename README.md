...
この2つを比較して、挿入箇所を黄色でハイライトした提出用Wordファイルを作ってください。
```
...
挿入された本文を黄色でハイライトし、変更履歴は承認して、ハイライトだけが残る提出用Wordファイルを作ってください。
```
...
$word-revision-highlighter を使ってください。
変更履歴つきWordファイルがあります。
既存のハイライトは消さずに、今回挿入された箇所だけを明るい緑で追加ハイライトしてください。
```
...
```text
$word-revision-highlighter を使ってください。
変更履歴つきWordファイルから、挿入箇所は黄色ハイライト、削除箇所は本文に戻して赤い取り消し線で表示する提出用ファイルを作ってください。
```
...
Please create a submission-ready Word file with inserted revisions highlighted in yellow.
```

```text
Use the word-revision-highlighter workflow.
I already have a Word file with Track Changes.
Please preserve existing highlights and add bright green highlights to the newly inserted text.
```
